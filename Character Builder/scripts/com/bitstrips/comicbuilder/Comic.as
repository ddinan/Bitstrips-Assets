package com.bitstrips.comicbuilder
{
   ...
   public class Comic extends EventDispatcher
   {
         ...
         if(comicData.editable)
         {
            if(selectedStrip != null && selectedPanel != null)
            {
               ...
               if(_loc2_.length < Comic.PANEL_MAX)
               {
                  ...
                  {
                     _loc6_ = Comic.PANEL_WIDTH_MIN * 2 + Comic.BORDER_SIZE / 2 - selectedPanel.panel_width;
                     _loc7_ = 0;
                  ...
                  this.comic_width = selectedStrip.strip_width;
               }
            }
         }
      }
      ...
      public function borderResizeComicWidth() : void
      {
         ...
         var _loc1_:Number = currentBorder.mouseX - borderOffset.x;
      ...
      public function removeStrip_click(param1:MouseEvent) : void
      {
         ...
         if(comicData.editable)
         {
            if(stripList.length > Comic.STRIP_MIN && selectedStrip != null)
            {
      ...
      public function removePanel_click(param1:MouseEvent) : void
      {
         ...
         if(comicData.editable)
         {
            if(selectedStrip != null && selectedPanel != null)
            {
               if(selectedStrip.panels.length > Comic.STRIP_MIN)
               {
                  pre_undo();
      ...
      {
         return max_current_panels() * (PANEL_WIDTH_MIN + Comic.BORDER_SIZE) + Comic.BORDER_SIZE;
      }
      ...
      {
         var _loc3_:ComicStrip = null;
         var _loc1_:Number = ComicStrip(stripList[0]).strip_width;
         var _loc2_:int = 1;
         while(_loc2_ < stripList.length)
         {
            _loc3_ = stripList[_loc2_];
      ...
      {
         return comicData["bkgrColor"];
      }
      
      public function get series_id() : int
      {
         if(comicData && comicData.hasOwnProperty("series"))
         {
