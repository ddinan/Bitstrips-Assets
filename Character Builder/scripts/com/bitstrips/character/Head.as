package com.bitstrips.character
{
   ...
   public class Head extends HeadBase implements IHead
   {
      ...
      public function Head(param1:PPData = undefined, param2:ColourData = undefined)
      {
         ...
         cld.addEventListener("NEW_COLOUR",update_colours,false,0,true);
         ...
         for(_loc4_ in _loc3_)
         {
            ...
            _loc9_ = init_head_piece(_loc8_);
            piece_dict[_loc8_] = _loc9_;
            if(_loc9_ != null)
            {
               pieces.push(_loc9_);
               addChild(_loc9_);
               ...
               {
                  hair_back = init_head_piece(_loc8_);
               }
               this.bounds_test();
            }
         }
         ...
         for(_loc6_ in _loc5_["x_syms"])
         {
            ...
            piece_dict[_loc10_[0]].x_sym = piece_dict[_loc10_[1]];
            piece_dict[_loc10_[1]].x_sym = piece_dict[_loc10_[0]];
         }
         for(_loc6_ in _loc5_["x_locked"])
         {
            _loc7_ = _loc5_["x_locked"][_loc6_];
            piece_dict[_loc7_].x_locked = 1;
         }
         for(_loc6_ in _loc5_["y_locked"])
         {
            _loc7_ = _loc5_["y_locked"][_loc6_];
            piece_dict[_loc7_].y_locked = 1;
         }
         if(debug)
         {
         ...
         for(_loc6_ in _loc5_["x_scales"])
         {
            ...
            {
               _loc12_.push(piece_dict[_loc11_[_loc13_]]);
            }
            if(debug)
            {
               trace("X scale: " + _loc6_ + " " + _loc12_);
            }
            _loc12_.push(hair_back);
            for(_loc13_ in _loc11_)
            {
               piece_dict[_loc11_[_loc13_]].x_scales = _loc12_;
            }
         }
         for(_loc6_ in _loc5_["locked"])
         {
            ...
            {
               _loc14_.push(piece_dict[_loc11_[_loc15_]]);
            }
            piece_dict[_loc6_].locked = _loc14_;
         }
         ...
         {
            set_hairstyle(param1["hairstyle"]);
            if(param1["beardstyle"])
            {
               set_beardstyle(param1["beardstyle"]);
            }
            set_backstyle(param1["backstyle"]);
         }
         set_rotation(h_rot);
      ...
      public function set edit(param1:Boolean) : void
      {
         ...
         else
         {
            for(_loc2_ in pieces)
            {
               ...
               pieces[_loc2_].addEventListener(MouseEvent.MOUSE_DOWN,piece_down,false,0,true);
               pieces[_loc2_].addEventListener(MouseEvent.ROLL_OVER,piece_over,false,0,true);
               pieces[_loc2_].addEventListener(MouseEvent.ROLL_OUT,piece_out,false,0,true);
            }
      ...
      private function init_head_piece(param1:String) : HeadPiece
      {
         ...
         if(_edit == true)
         {
            _loc5_.addEventListener(MouseEvent.MOUSE_DOWN,piece_down,false,0,true);
            _loc5_.addEventListener(MouseEvent.ROLL_OVER,piece_over,false,0,true);
            _loc5_.addEventListener(MouseEvent.ROLL_OUT,piece_out,false,0,true);
         }
         return _loc5_;
      }
      ...
      public function update_colours(param1:Event = null) : void
      {
         ...
         for(_loc2_ in pieces)
         {
            if(debug)
            {
               trace("\t" + _loc2_ + " -- " + pieces[_loc2_]);
            }
            if(pieces[_loc2_].artwork != null)
            {
               _loc3_ = _cld.colour_clip(pieces[_loc2_].artwork);
               if(_loc3_ == -1)
               {
                  if(debug)
                  {
                     trace("Failed colour: " + pieces[_loc2_].piece_id);
                  }
               }
            }
         }
         ...
         _cld.colour_clip(hair_back.artwork);
      }
